...
def replace_index_and_read_frame(frame_dir, idx, size, frame_type, scene_info=None, tracking=False):
    if frame_type == "image":
        new_frame_dir = frame_dir[0:-22] + str(int(frame_dir[-22:-16]) + idx).zfill(6) + frame_dir[-16::]
        load_type = Image.BICUBIC
    elif frame_type == "seg_mask":
        new_frame_dir = frame_dir[0:-17] + str(int(frame_dir[-17:-11]) + idx).zfill(6) + frame_dir[-11::]
        load_type = Image.NEAREST
    else:
        new_frame_dir = frame_dir[0:-29] + str(int(frame_dir[-29:-23]) + idx).zfill(6) + frame_dir[-23::]
        load_type = Image.NEAREST
    try:
    ...
    for scene_instance in glob.glob(scene + "*.txt"):
        with open(scene_instance, "r") as file_instance:
            ...
            for idx, frame_info in enumerate(inst_info):
                ...
                x_l = float(frame_info[0]) / 2048 * size[1]
                x_l_padded = max(x_l - 15, 0)
                x_r = (float(frame_info[0]) + float(frame_info[2])) / 2048 * size[1]
                x_r_padded = min(x_r + 15, size[1])
                y_t = float(frame_info[1]) / 1024 * size[0]
                y_t_padded = max(y_t - 10, 0)
                y_b = (float(frame_info[1]) + float(frame_info[3])) / 1024 * size[0]
                y_b_padded = min(y_b + 10, size[0])
                bbox_size = np.array([float(frame_info[3]) / 1024, float(frame_info[2]) / 2048])  # y,x
                roi = [x_l, x_r, y_t, y_b]
                ...
                if idx >= config["train_params"]["num_input_frames"] and config["test_params"]["lambda_traj"] > 1:
                    ...
                    x_l_start = float(frame_info_start[0]) / 2048 * size[1]
                    x_r_start = (float(frame_info_start[0]) + float(frame_info_start[2])) / 2048 * size[1]
                    ...
                    x_l = (float(frame_info[0]) / 2048 * size[1]) + x_displacement
                    x_l_padded = max(x_l - 15, 0)
                    x_r = ((float(frame_info[0]) + float(frame_info[2])) / 2048 * size[1]) + x_displacement
                    x_r_padded = min(x_r + 15, size[1])
                    roi = [x_l, x_r, y_t, y_b]
                    roi_padded = [x_l_padded, x_r_padded, y_t_padded, y_b_padded]
...
def complete_full_list(image_dir, num_frames, output_name):
    dir_list = [image_dir[0:-22] + str(int(image_dir[-22:-16]) + i).zfill(6) + '_' + output_name for i in
                range(num_frames)]
    return dir_list
...
def load_optical_flow_occlusion_mask(bw_optical_flow_dir, bw_occlusion_dir, fw_optical_flow_dir, fw_occlusion_dir,
                                     size, num_frame, use_fw_of):
    out_dict = {}
    ...
    for i in range(1, num_frame):
        new_dir_backward_optical_flow = bw_optical_flow_dir[0:-37] +\
            str(int(bw_optical_flow_dir[-37:-31]) + i).zfill(6) +\
            bw_optical_flow_dir[-31::]
        new_dir_backward_occlusion_mask = bw_occlusion_dir[0:-35] +\
            str(int(bw_occlusion_dir[-35:-29]) + i).zfill(6) +\
            bw_occlusion_dir[-29::]
        ...
        if use_fw_of:
            new_dir_forward_optical_flow = fw_optical_flow_dir[0:-36] + \
                str(int(fw_optical_flow_dir[-36:-30]) + i).zfill(6) + \
                fw_optical_flow_dir[-30::]
            new_dir_forward_occlusion_mask = fw_occlusion_dir[0:-35] + \
                str(int(fw_occlusion_dir[-35:-29]) + i).zfill(6) +\
                fw_occlusion_dir[-29::]
    ...
    out_dict["target_bw_of"] = torch.stack(backward_optical_flows, dim=1)
    out_dict["target_bw_occ"] = clip_mask(torch.stack(backward_occlusion_masks, dim=1))
    if use_fw_of:
        out_dict["target_fw_of"] = torch.stack(forward_optical_flows, dim=1)
        out_dict["target_fw_occ"] = clip_mask(torch.stack(forward_occlusion_masks, dim=1))
    return out_dict
...
class Cityscapes(Dataset):
    def __init__(self, images_path=None, segmasks_path=None, instances_path=None, tracking_path=None, bw_occ_path=None,
        ...
        self.use_fw_of = config["train_params"].get("use_fw_of", True)
    ...
    def __getitem__(self, idx):
        ...
        mask_dir = os.path.join(self.segmask_root, self.datalist[idx].strip()[0:-15] + self.segmask_suffix)
        instance_dir = os.path.join(self.instance_root, self.datalist[idx].strip()[0:-15] + self.instance_suffix)
        tracking_dir = os.path.join(self.tracking_root, self.datalist[idx].strip()[0:-15])
