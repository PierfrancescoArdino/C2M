...
def main():
    ...
    num_images = 500 if phase=="val" else 2975
    # create dataset
    if not os.path.exists(dataset_root + f"/dataset_{phase}_new.pickle"):
        dataset = DatasetFactory.create_dataset(name=args.dataset,
                                            dataset_root=dataset_root,
                                            load_img=False, num_images=num_images, phase=phase)

        import pickle
        outfile = open(dataset_root + f"/dataset_{phase}_new.pickle",'wb')
        pickle.dump(dataset,outfile)
        outfile.close()
    else:
        import pickle
        infile = open(dataset_root + f"/dataset_{phase}_new.pickle",'rb')
    ...
    for v_idx, video in enumerate(dataset):
        ...
        if not skip:
            ...
            model_path = os.path.join(f'results_{phase}_new', args.dataset, model_name)
            if not os.path.isdir(model_path):
                os.makedirs(model_path)
            result_path = os.path.join(model_path, f'{video.img_names[0].split("/")[-1][:-16]}_{video.instance_id}.txt')
